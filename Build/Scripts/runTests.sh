...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/pxa_product_manager/${1}"
else
    case ${TEST_SUITE} in
        functional)
            TEST_FILE="Web/typo3conf/ext/pxa_product_manager/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/pxa_product_manager/Tests/Unit"
            ;;
    esac
fi
