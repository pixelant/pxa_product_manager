...
class CategoryRepository extends \TYPO3\CMS\Extbase\Domain\Repository\CategoryRepository
{
     ...
     * @param array $uids
     * @param array $result
     * @param array $uniqueUids
     * @return array
     */
    public function getParentLevelTreeByUids(array $uids = [], array $result = [], array $uniqueUids = []) : array
    {
        if (empty($uids)) {
            return $result;
        }
        ...
        $level = !empty($result)
            ? max(array_column($result, 'level')) + 1
            : 1;

        foreach ($uids as $uid) {
            if (!in_array($uid, $uniqueUids)) {
                $uniqueUids[] = $uid;
                $currentLevelResults[] = ['level' => $level, 'uid' => $uid];
            }
        }

        $result = array_merge($result, $currentLevelResults);
        array_unique($result);

        $parents = $this->getParentUidsByUidList($uids);

        $parents = array_filter($parents, function ($uid) {
            return $uid != 0;
        });

        $parents = array_filter($parents, function ($uid) use ($uniqueUids) {
            return !in_array($uid, $uniqueUids);
        });
        ...
        return $this->getParentLevelTreeByUids($parents, $result, $uniqueUids);
    }
     ...
     * @return array
     */
    public function getParentUidsByUidList(array $uids = []) : array
    {
        /** @var QueryBuilder $queryBuilder */
        $queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable(
            'sys_category'
        ...
        return array_column($rows, 'parent');
    }

    public function getCategoriesLevelTreeForProduct(int $productUid = 0)
    {
        if ($productUid <= 0) {
            return [];
        }

        $levelTree = $this->getParentLevelTreeByUids(
            $this->getProductsCategoriesUids([$productUid])
        );

        $sortedUidList = array_column($levelTree, 'uid');
        $categories = $this->findByUidList($sortedUidList);
        $categoryKeys = array_map(function ($category) {
            return $category->getUid();
        }, $categories);

        $categories = array_combine($categoryKeys, $categories);

        foreach ($levelTree as $key => $leveTreeItem) {
            $levelTree[$key]['category'] = $categories[$leveTreeItem['uid']];
        }
